...
class AbstractLane(object):
    ...
    def on_lane(
        ...
        is_on = (
            abs(lateral) <= self.width_at(longitudinal) / 2 + margin
            and -VEHICLE_LENGTH <= longitudinal < self.length + VEHICLE_LENGTH
        )
        return is_on
    ...
    def distance_between_points(self, position1: np.ndarray, position2: np.ndarray):
        ...
        return (
            self.local_coordinates(position2)[0] - self.local_coordinates(position1)[0]
        )
...
class CommonRoadLane(AbstractLane):
    """A lane defined by the commonroad standard"""
    ...
    def __init__(
        ...
        :param lanelet: the commonroad lanelet associated with this lane
        :param ring: if the lanelet forms a closed ring
    ...
    def position(self, longitudinal: float, lateral: float) -> np.ndarray:
        ...
        if closest_vertex_index == len(self.lanelet.center_vertices) - 1:
            ...
            vertex2 = self.lanelet.center_vertices[closest_vertex_index, :]
        else:
...
class CircularLane(AbstractLane):
    ...
    def distance_with_heading(self, position: np.ndarray, heading: Optional[float]):
        ...
        in_range = (phi - self.start_phase) % (2 * np.pi) <= (
